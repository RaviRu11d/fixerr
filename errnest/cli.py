"""``errnest`` command-line interface.
...
config_app = typer.Typer(help="View and change errnest configuration.", no_args_is_help=True)
...
def capture(
    ...
    console.print(
        ...
        f"When fixed: [cyan]errnest resolve {error_id} \"...\"[/cyan]."
    )
...
def edit(error_id: int = typer.Argument(..., help="Error id.")) -> None:
    ...
    empty) to cancel. Marks the error resolved, same as `errnest resolve`.
    """
    import os
    import shlex
    import subprocess
    ...
    fd, path = tempfile.mkstemp(prefix=f"errnest-fix-{error_id}-", suffix=".md")
    try:
...
def _surface_panel(command: str, matches: List[Match]):
    ...
    if not matches:
        ...
        return Panel(body, title="errnest", border_style="cyan", expand=False)
    ...
    body.append(
        f"errnest show <id>  for full details · errnest resolve <id> \"...\" "
        f"to update  [{ids}]",
        style="dim italic",
    )
    return Panel(body, title="errnest", border_style="cyan", expand=False)
...
# `eval "$(errnest shell-init ...)"`, and goes stale until re-sourced after a
...
_ZSH_HOOK = r"""# errnest Tier 1: auto-capture every failed command.
__errnest_last_cmd=""
__errnest_preexec() { __errnest_last_cmd="$1" }

__errnest_ignore=(__ERRNEST_IGNORE_ARRAY__)
__errnest_min_exit_code=__ERRNEST_MIN_EXIT_CODE__

TRAPZERR() {
  local __ec=$?
  local __cmd="$__errnest_last_cmd"
  [[ -z "$__cmd" ]] && return
  [[ "$__cmd" == errnest* ]] && return
  (( __ec < __errnest_min_exit_code )) && return
  local __skip
  for __skip in "${__errnest_ignore[@]}"; do
    [[ "$__cmd" == "$__skip"* ]] && return
  done
  ( command errnest _auto_capture \
      --exit-code "$__ec" \
      --command "$__cmd" \
      --cwd "$PWD" \
...
add-zsh-hook preexec __errnest_preexec
"""

_BASH_HOOK = r"""# errnest Tier 1: auto-capture every failed command.
...
__errnest_last_ec=0
trap '__errnest_last_ec=$?' ERR

__errnest_ignore=(__ERRNEST_IGNORE_ARRAY__)
__errnest_min_exit_code=__ERRNEST_MIN_EXIT_CODE__

__errnest_should_skip() {
...
if [[ -f "$HOME/.bash-preexec.sh" ]]; then
  ...
  __errnest_preexec() { __errnest_last_cmd="$1"; }
  preexec_functions+=(__errnest_preexec)
  __errnest_precmd() {
    local __ec=$__errnest_last_ec
    __errnest_last_ec=0
    [[ $__ec -eq 0 ]] && return
    (( __ec < __errnest_min_exit_code )) && return
    local __cmd="$__errnest_last_cmd"
    [[ -z "$__cmd" ]] && return
    [[ "$__cmd" == errnest* ]] && return
    __errnest_should_skip "$__cmd" && return
    ( command errnest _auto_capture \
        --exit-code "$__ec" \
        --command "$__cmd" \
        --cwd "$PWD" \
  ...
  precmd_functions+=(__errnest_precmd)
else
  __errnest_last_cmd=""
  ...
  trap '[[ "$BASH_COMMAND" != __errnest_* ]] && __errnest_last_cmd=$BASH_COMMAND' DEBUG
  ...
  PROMPT_COMMAND="__errnest_precmd${PROMPT_COMMAND:+; $PROMPT_COMMAND}"
fi
"""


_PWSH_HOOK = r"""# errnest Tier 1: auto-capture every failed command.
...
if (-not $global:__errnest_hook_installed) {
    $global:__errnest_hook_installed = $true

    $global:__errnest_original_prompt = if (Test-Path Function:\prompt) {
        $function:prompt
    } else {
        { "PS $($executionContext.SessionState.Path.CurrentLocation)$('>' * ($nestedPromptLevel + 1)) " }
    }

    $global:__errnest_ignore = __ERRNEST_IGNORE_ARRAY__
    $global:__errnest_min_exit_code = __ERRNEST_MIN_EXIT_CODE__
    $global:__errnest_prev_lastexitcode = $null
    ...
    function global:__errnest_quote_arg($s) {
        if ($null -eq $s) { $s = '' }
        '"' + ($s -replace '"', '""') + '"'
    }
    ...
    function global:prompt {
        ...
        if ((-not $__succeeded) -and (-not $__wasInterrupted)) {
            ...
            if ($__cmd) {
                $__skip = $false
                if ($__cmd -like 'errnest*') { $__skip = $true }
                foreach ($__ignored in $global:__errnest_ignore) {
                ...
                if (-not $__skip) {
                    ...
                    $__ec = if ($null -ne $__lastexitcode_now -and
                                $__lastexitcode_now -ne $global:__errnest_prev_lastexitcode) {
                        $__lastexitcode_now
                    } else {
                        1
                    }

                    if ($__ec -ge $global:__errnest_min_exit_code) {
                        ...
                        # into separate tokens before errnest ever sees them,
                        ...
                        $__argsStr = "_auto_capture --exit-code $__ec " +
                            "--command $(__errnest_quote_arg $__cmd) " +
                            "--cwd $(__errnest_quote_arg $__cwd)"
                        if ($__gitCommit) {
                            $__argsStr += " --git-commit $(__errnest_quote_arg $__gitCommit)"
                        }

                        try {
                            Start-Process -FilePath 'errnest' -ArgumentList $__argsStr `
                                -NoNewWindow -PassThru -ErrorAction Stop | Out-Null
                        } catch {
                            # errnest not on PATH, or spawn failed — never
        ...
        $global:__errnest_prev_lastexitcode = $__lastexitcode_now

        & $global:__errnest_original_prompt
    }
}
"""
...
def shell_init(
    ...
    \b
      eval "$(errnest shell-init zsh)"   # in ~/.zshrc
      eval "$(errnest shell-init bash)"  # in ~/.bashrc
      errnest shell-init pwsh | Out-String | Invoke-Expression   # in $PROFILE
    ...
    if shell_name == "pwsh":
        ...
        script = _PWSH_HOOK.replace("__ERRNEST_IGNORE_ARRAY__", array_literal).replace(
            "__ERRNEST_MIN_EXIT_CODE__", str(min_exit_code)
        )
        print(script)
        return
    ...
    script = template.replace("__ERRNEST_IGNORE_ARRAY__", array_literal).replace(
        "__ERRNEST_MIN_EXIT_CODE__", str(min_exit_code)
    )
    print(script)
...
# ------------------------------------------------------------------- run ---
# Tier 2: `errnest run <command>` wraps a command, tees its stderr (terminal
...
def _run_result_panel(
    ...
    body.append(f"  (errnest resolve {error_id} \"...\" to add a fix)\n")
    if explaining:
        body.append(
            f"AI explanation generating in the background — check `errnest show {error_id}` "
            f"shortly.\n",
            style="dim",
        )
    if matches:
        ...
        for m in matches:
            ...
            if m.fix:
                fix_line = m.fix.splitlines()[0]
                body.append(f"  Fix: {fix_line[:70]}\n", style="dim")
    return Panel(body, title=f"errnest captured error (exit {returncode})", border_style="cyan", expand=False)
...
def run_wrapped(
    command_parts: List[str] = typer.Argument(
        ..., help="The command to run, e.g. `errnest run docker compose up`."
    ),
...
def _spawn_explain_worker(error_id: int) -> None:
    ...
    Uses ``sys.executable -m errnest.cli`` rather than the installed
    ``errnest`` binary so this works the same in an editable/dev install
    ...
    subprocess.Popen(
        [sys.executable, "-m", "errnest.cli", "_explain_worker", str(error_id)],
...
def explain(
    ...
    Check back with `errnest show <id>` or the dashboard.
    """
    from .store import Store
    ...
    console.print(
        ...
        f"check `errnest show {error_id}` shortly.[/dim]"
    )
...
def dashboard() -> None:
    ...
    from .dashboard import ErrnestDashboard

    ErrnestDashboard().run()
...
def config_set(
    ...
    """Set a single config value and persist it to ~/.errnest/config.toml."""
    try:
...
def doctor() -> None:
    ...
    console.print("[bold]errnest doctor[/bold]")
